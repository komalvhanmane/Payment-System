...
public class TransactionDao {
	public int mobileNoTransfer(String phno,int pin,int bank,String phnor, double amount) {
		...
		if(c!=null) {
			try {
				...
				PreparedStatement ps=con.prepareStatement(" update Account set acc_balance=? where acc_num = ( select acc_num from cust_acc where cust_phone = ? )  ");
				ps.setDouble(1,c.getA().getAcc_bal()-amount);
				ps.setString(2,phno);
				i=ps.executeUpdate();
				
				PreparedStatement ps1=con.prepareStatement("select * from Account where acc_num = ( select acc_num from cust_acc where cust_phone = ? )  ");
				...
				if(a!=null) {
					PreparedStatement ps2=con.prepareStatement(" update Account set acc_balance=? where acc_num = ( select acc_num from cust_acc where cust_phone = ? )  ");
